...
HTTP прокси работает так же, как SOCKS5 прокси, описанный выше, но использует Squid вместо Dante. Используйте его, если ваше программное обеспечение поддерживает только HTTP прокси (например, некоторые браузеры, CLI инструменты или корпоративные приложения, в которых нет поддержки SOCKS5).

Доступны два контейнера HTTP прокси:
- **`http-proxy-local.antizapret:3128`** — трафик выходит через **локальный** сервер
- **`http-proxy-world.antizapret:3128`** — трафик выходит через **зарубежный** сервер

HTTPS прокси порт (`3129` по умолчанию) также доступен с самоподписанным TLS сертификатом для безопасного внешнего доступа. Если вы используете эти прокси только внутри VPN, вам это, вероятно, не нужно.

Аутентификация: HTTP Basic (настраивается через переменные окружения).
Чтобы отключить аутентификацию, пропустите `HTTP_PROXY_USERNAME` и `HTTP_PROXY_PASSWORD` (или оставьте их пустыми).

> [!CAUTION]
> При использовании HTTP (не HTTPS) прокси с доступом из Интернета, учетные данные передаются в виде открытого текста и могут быть перехвачены. Чтобы безопасно ограничить доступ, используйте HTTPS порт (`3129`) или оставьте HTTP порт (`3128`) доступным только через VPN.

### Конфигурация

Пример доступен в [`docker-compose.override.sample.yml`](docker-compose.override.sample.yml).

> [!NOTE]
> `http-proxy-world` требует [режим Docker Swarm](#docker-swarm-несколько-узлов-выхода-продвинутый) с двумя узлами.
> На одном сервере будет работать только `http-proxy-local`.

### Настройка клиента

1. Подключитесь к VPN
2. Настройте HTTP прокси в вашем приложении или браузере:
    - **Хост:** `http-proxy-local.antizapret` или `http-proxy-world.antizapret`
    - **Порт:** `3128` (HTTP) или `3129` (HTTPS)
    - **Имя пользователя:** значение `HTTP_PROXY_USERNAME`
    - **Пароль:** значение `HTTP_PROXY_PASSWORD`
...
### HTTP Прокси
- `HTTP_PROXY_USERNAME` - имя пользователя для HTTP аутентификации (пропуск отключает аутентификацию)
- `HTTP_PROXY_PASSWORD` - пароль для HTTP аутентификации (пропуск отключает аутентификацию)
- `HTTP_PROXY_PORT=3128` - HTTP порт для прослушивания
- `HTTPS_PROXY_PORT=3129` - HTTPS порт для прослушивания
