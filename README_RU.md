...
Файлы `exclude-hosts-dist.txt` и `exclude-hosts-custom.txt` также загружаются в AdGuard как исключения для DNS rewrite. Для совпавшего домена AdGuard возвращает реальный адрес вместо маршрутизации через соответствующий VPN-узел. Шаблоны используют синтаксис расширенных регулярных выражений; также поддерживаются выражения, уже обрамлённые символами `/`. Локальные и зарубежные правила загружаются из соответствующих контейнеров `az-local` и `az-world` и ограничиваются одноимённым клиентом AdGuard.
