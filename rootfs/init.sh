...
function set_subnets_udp () {
    ...
    #ferm.conf
    sed -i -E "s|^(\@def \$VPNUDP_RANGE.*= ).*|\1$1|1" /etc/ferm/ferm.conf
    sed -i -E "s|^(\@def \$VPNUDP_DNS.*= ).*|\1$OPENVPN_UDP_DNS|1" /etc/ferm/ferm.conf
...
#ferm.conf
    sed -i -E "s|^(\@def \$VPNTCP_RANGE.*= ).*|\1$1|1" /etc/ferm/ferm.conf
    sed -i -E "s|^(\@def \$VPNTCP_DNS.*= ).*|\1$OPENVPN_TCP_DNS|1" /etc/ferm/ferm.conf 
